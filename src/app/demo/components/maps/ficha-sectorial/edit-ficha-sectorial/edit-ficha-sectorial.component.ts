...
fichaSectorialForm: FormGroup<any>;
  ...
  ngOnInit(): void {
    ...
    this.check.EditFichaSectorialComponent = this.helper.decryptData('EditFichaSectorialComponent') || false;
    this.check.EditFichaAll = this.helper.decryptData('EditFichaAll') || false;
  ...
  obtenerficha() {
    this.deshabilitarCampo('direccion_geo');
    this.filter.obtenerActividadProyecto(this.token,this.id).subscribe(response => {
      if (response.data) {
        ...
        for (const key in ficha) {
          if (Object.prototype.hasOwnProperty.call(ficha, key)) {
            ...
            if (campo) {
              ...
              if (key!='observacion'&&key!='estado' && !this.check.EditFichaAll ) {                
